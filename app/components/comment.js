...
export default class Comment extends React.Component {
  ...
  */
  ...
  this.setState({likeCommentCounter: updatedLikeCounter});
  };
  if (this.didUserLike()) {
  ...
  */
  didUserLike() {
  var likeCommentCounter = this.state.likeCommentCounter;
  var liked = false;
  // Look for a likeCounter entry with userId 4 -- which is the
  // current user.
  for (var i = 0; i < likeCommentCounter.length; i++) {
  if (likeCommentCounter[i]._id === 4) {
  liked = true;
  break;
  }
  }
